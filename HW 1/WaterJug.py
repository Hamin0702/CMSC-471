...
def AStar(target):
    ...
    # Lists of the Jugs states
    queue = []
    queue.append(start)
    
    graph = []
    graph.append(start.jugs)
    visited = []
    path = []
    moves = []
    ...
    output.write("Serching strategy: BFS\n")
    ...
    while (len(queue) > 0):
        ...
        if(current.jug1 == 0 and current.jug2 == 0):
            continue
        ...
        if(current.jug1 != 0 and current.jug2 < j2cap):
            ...
            if((jug1,jug2) not in visited):
                queue.append(Jug(jug1,jug2,1,current))
                graph.append((jug1,jug2))
        ...
        if(current.jug1 < j1cap and current.jug2 != 0):
            ...
            if((jug1,jug2) not in visited):
                queue.append(Jug(jug1,jug2,2,current))
                graph.append((jug1,jug2))

        # Case 3: Empty Jug 1
        if(current.jug1 > 0 and (0,current.jug2) not in visited):
            queue.append(Jug(0,current.jug2,3,current))
            graph.append((0,current.jug2))

        # Case 4: Empty Jug 2
        if(current.jug2 > 0 and (current.jug1,0) not in visited):
            queue.append(Jug(current.jug1,0,4,current))
            graph.append((current.jug1,0))

    # Finding the path and the moves
    traverse = goal
    path.append(traverse.jugs)
    moves.append(traverse.caseNum)
    while(True):
        if(traverse.parent.isStart):
            traverse = traverse.parent
            path.append(traverse.jugs)
    ...
    delimiter = ','
    output.write("Path: ")
    output.write(delimiter.join([str(state) for state in path]))

    output.write("\nAction: ")
    # Moves to get to the solution
    for i in moves:
        if(i == 1):
            output.write("Pour 1, ")
        if(i == 2):
